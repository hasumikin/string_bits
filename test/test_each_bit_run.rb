...
class TestBitCountRun < Minitest::Test
  ...
  def test_low_nibble_zeros
    ...
    assert_equal 4, "\xF0".bit_run_count(1, 4)
  end

  def test_single_set_bit
    # 0x01 = 00000001: bit 0 is 1, bit 1 is 0
    assert_equal 1, "\x01".bit_run_count(1, 0)
  end

  def test_cross_byte_run
    # "\xFF\xFF\x00": 16 ones then 8 zeros
    assert_equal 16, "\xFF\xFF\x00".bit_run_count(1, 0)
    assert_equal 8,  "\xFF\xFF\x00".bit_run_count(0, 16)
  end
  ...
  def test_run_starting_mid_byte
    ...
    assert_equal 2, "\xF0".bit_run_count(0, 2)
  end
  ...
  def test_bit_mismatch_returns_nil
    ...
    assert_nil "\x00".bit_run_count(1, 0)
  end

  def test_out_of_range_returns_nil
    assert_nil "\xFF".bit_run_count(1, 8)
    assert_nil "\xFF".bit_run_count(0, 100)
  end
  ...
  def test_beyond_string_returns_nil
    assert_nil "\xFF".bit_run_count(1, 2**62)
    assert_nil "\xFF".bit_run_count(1, 2**64 - 1)
  end
  ...
  def test_negative_bignum_returns_nil
    ...
    assert_nil "\xFF".bit_run_count(1, -(2**100))
  end

  def test_unrepresentable_raises_argument_error
    assert_raises(ArgumentError) { "\xFF".bit_run_count(1, 2**64) }
  end

  def test_boolean_bit_values
    # true/false are accepted as aliases for the 1/0 that each_bit_run yields
  ...
  def test_long_run_ones
    ...
    assert_equal 800, data.bit_run_count(1, 0)
  end
  ...
  def test_long_run_starting_mid_byte
    ...
    assert_equal 796 + 4, data.bit_run_count(1, 4)  # 796 ones in middle bytes + 4 in last byte
  end
...
class TestEachBitRun < Minitest::Test
  ...
  def test_all_ones
    assert_equal [[1, 0, 8]], "\xFF".each_bit_run.to_a
  end

  def test_all_zeros
    assert_equal [[0, 0, 8]], "\x00".each_bit_run.to_a
  end

  def test_two_runs
    # 0xF0 = 11110000: bits 0-3 = 0, bits 4-7 = 1
    assert_equal [[0, 0, 4], [1, 4, 4]], "\xF0".each_bit_run.to_a
  end

  def test_alternating_bits
    # 0xAA = 10101010: bit0=0,bit1=1,...
    expected = (0..7).map { |i| [i.even? ? 0 : 1, i, 1] }
    assert_equal expected, "\xAA".each_bit_run.to_a
  end

  def test_cross_byte_run
    assert_equal [[1, 0, 8], [0, 8, 8]], "\xFF\x00".each_bit_run.to_a
  end

  def test_multi_byte_single_run
    assert_equal [[1, 0, 24]], "\xFF\xFF\xFF".each_bit_run.to_a
  end

  # --- lsb_first: false ---

  def test_msb_all_ones
    assert_equal [[1, 0, 8]], "\xFF".each_bit_run(lsb_first: false).to_a
  end

  def test_msb_two_runs
    # 0xF0 MSB-first: positions 0-3 are 1 (high nibble), positions 4-7 are 0
    assert_equal [[1, 0, 4], [0, 4, 4]], "\xF0".each_bit_run(lsb_first: false).to_a
  end

  def test_msb_can_merge_runs_across_byte_boundaries
    data = "\x0F\xF0"
    assert_equal [[0, 0, 4], [1, 4, 8], [0, 12, 4]], data.each_bit_run(lsb_first: false).to_a
  end
  ...
  def test_long_run_single
    ...
    assert_equal [[1, 0, 1024]], data.each_bit_run.to_a
  end
  ...
  def test_enumerator_to_a
    ...
    assert_equal [[1, 0, 8], [0, 8, 8]], e.to_a
  end
  ...
  def test_bit_run_count_msb_positions
    ...
    assert_equal 8, data.bit_run_count(1, 4, lsb_first: false)
    assert_equal 4, data.bit_run_count(0, 12, lsb_first: false)
  end
  ...
  def test_bit_offset_byte_aligned
    ...
    assert_equal [[0, 8, 8]], "\xFF\x00".each_bit_run(8).to_a
  end

  def test_bit_offset_non_byte_aligned
    # "\xF0" lsb: bits 0-3 are 0, bits 4-7 are 1; starting at 4 yields one run of 4 ones at offset 4
    assert_equal [[1, 4, 4]], "\xF0".each_bit_run(4).to_a
  end
  ...
  def test_bit_offset_msb
    # "\xF0" msb: bits 0-3 are 1, bits 4-7 are 0; starting at 4 yields one run of 4 zeros at offset 4
    assert_equal [[0, 4, 4]], "\xF0".each_bit_run(4, lsb_first: false).to_a
  end
