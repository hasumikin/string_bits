...
class TestEachBitOffset < Minitest::Test
  ...
  def test_msb_set_positions
    positions = []
    @data.each_bit_offset(1, lsb_first: false) { |i| positions << i }
    assert_equal [0, 2, 4, 6, 8, 9, 12, 13], positions
  end

  def test_set_with_true_alias
    positions = []
    @data.each_bit_offset(true) { |i| positions << i }
    assert_equal [1, 3, 5, 7, 10, 11, 14, 15], positions
  end

  # --- bit=0 / false (unset bits) ---
  ...
  def test_msb_unset_positions
    positions = []
    @data.each_bit_offset(0, lsb_first: false) { |i| positions << i }
    assert_equal [1, 3, 5, 7, 10, 11, 14, 15], positions
  end

  def test_unset_with_false_alias
    positions = []
  ...
  def test_set_and_unset_partition_all_positions
    set_positions   = @data.each_bit_offset(1).to_a
    unset_positions = @data.each_bit_offset(0).to_a
  ...
  def test_empty_string
    assert_empty "".each_bit_offset(1).to_a
    assert_empty "".each_bit_offset(0).to_a
  end

  def test_all_zeros_set
    assert_empty "\x00\x00".each_bit_offset(1).to_a
  end

  def test_all_zeros_unset
    assert_equal (0..15).to_a, "\x00\x00".each_bit_offset(0).to_a
  end

  def test_all_ones_set
    assert_equal (0..7).to_a, "\xFF".each_bit_offset(1).to_a
  end

  def test_all_ones_unset
    assert_empty "\xFF".each_bit_offset(0).to_a
  end

  def test_single_bit_msb_high
    positions = []
    "\x80".each_bit_offset(1) { |i| positions << i }
    assert_equal [7], positions
  end

  def test_single_bit_msb_low
    positions = []
    "\x01".each_bit_offset(1) { |i| positions << i }
  ...
  def test_returns_enumerator_without_block
    assert_instance_of Enumerator, @data.each_bit_offset(1)
    assert_instance_of Enumerator, @data.each_bit_offset(0)
  end

  def test_returns_self_with_block
    assert_same @data, @data.each_bit_offset(1) {}
    assert_same @data, @data.each_bit_offset(0) {}
  end
  ...
  def test_msb_reverses_bit_numbering_within_each_byte_set
    msb = @data.each_bit_offset(1, lsb_first: false).to_a
    lsb = @data.each_bit_offset(1, lsb_first: true).to_a
    assert_equal msb, lsb.map { |n| (n & ~7) | (7 - (n & 7)) }.sort
  end

  def test_msb_and_lsb_cover_same_count
    assert_equal @data.each_bit_offset(1).to_a.size,
                 @data.each_bit_offset(1, lsb_first: false).to_a.size
  end

  def test_set_and_unset_counts_match_bit_count
    set_count   = @data.each_bit_offset(1).to_a.size
    unset_count = @data.each_bit_offset(0).to_a.size
  ...
  def test_bit_offset_non_byte_aligned
    ...
    assert_equal [4, 5, 6, 7], "\xF0".each_bit_offset(1, 4).to_a
  end

  def test_bit_offset_zero_same_as_default
    assert_equal @data.each_bit_offset(1).to_a, @data.each_bit_offset(1, 0).to_a
  end

  def test_bit_offset_at_total_bits_yields_nothing
    assert_empty "\xFF".each_bit_offset(1, 8).to_a
  end

  def test_bit_offset_beyond_total_bits_yields_nothing
    assert_empty "\xFF".each_bit_offset(1, 100).to_a
  end

  def test_bit_offset_msb
    # "\xF0" msb: set bits at 0,1,2,3; starting at 4 yields nothing (unset)
    assert_empty "\xF0".each_bit_offset(1, 4, lsb_first: false).to_a
    assert_equal [4, 5, 6, 7], "\xF0".each_bit_offset(0, 4, lsb_first: false).to_a
  end

  def test_bit_offset_negative_raises_index_error
    assert_raises(IndexError) { "\xFF".each_bit_offset(1, -1).to_a }
  end

  def test_bit_offset_beyond_string_yields_nothing
    assert_empty "\xFF".each_bit_offset(1, 2**62).to_a
  end

  def test_unrepresentable_bit_offset_raises_argument_error
    assert_raises(ArgumentError) { "\xFF".each_bit_offset(1, 2**64).to_a }
  end
end
