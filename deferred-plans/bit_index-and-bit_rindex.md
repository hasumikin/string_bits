...
`bit` accepts `0`, `1`, `false`, or `true` (`false`/`true` are accepted as aliases for `0`/`1`), matching the values used by `each_bit_offset` and `bit_run_count`.
...
data.bit_index(1)      #=> 4    (first set bit)
data.bit_index(0)     #=> 0    (first clear bit)
data.bit_index(1, 5)   #=> 5    (first set bit at or after position 5)
"\x00".bit_index(1)    #=> nil  (no set bit)

# lsb_first: false changes which physical bit each position refers to
"\xF0".bit_index(1, lsb_first: false)   #=> 0   (MSB-first: bits 0-3 are 1)
"\xF0".bit_index(0, lsb_first: false)  #=> 4
```
...
data.bit_rindex(1)     #=> 7    (last set bit)
data.bit_rindex(0)    #=> 3    (last clear bit)
data.bit_rindex(1, 5)  #=> 5    (last set bit at or before position 5)
```

### Use case --- bitmap allocator (find first free, find next free)

The core scan of a first-fit or next-fit allocator. `bit_index(0)` is the "find first zero bit" primitive; passing a cursor turns it into "find next free bit" without restarting at position 0:

```ruby
free = bitmap.bit_index(0)              # first free block, or nil if full
free = bitmap.bit_index(0, last_alloc)  # next-fit: scan forward from cursor
```

`bit_rindex(1)` recovers the high-water mark --- the position of the last allocated block --- for trimming or compaction:

```ruby
high_water = bitmap.bit_rindex(1)        # nil if the bitmap is entirely free
```

Compared with `each_bit_offset(0).first`, the scalar form allocates no `Enumerator`, which is what makes it viable in an AOT-compiled hot loop.
...
```ruby
first_owned = bitfield.bit_index(1, lsb_first: false)
```
